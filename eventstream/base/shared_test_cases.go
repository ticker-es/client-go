...
func EventStreamSampleGroup(factory func() EventStream) {
	...
	It("streams the EventStream correctly", func() {
		...
		w.Stream().Stream(ctx, Select(), Range(2, 3), func(e *Event) error {
	...
	It("Stream respects the EventStream's LastSequence when lower Bracket is too small", func() {
		...
		var events []*Event
		ctx := context.Background()
		w.Stream().Stream(ctx, Select(), Range(-1, 2), func(e *Event) error {
			events = append(events, e)
			return nil
		})
		Expect(len(events)).To(Equal(2))
	...
	It("Stream respects the EventStream's LastSequence when upper Bracket is too high", func() {
		...
		var events []*Event
		ctx := context.Background()
		w.Stream().Stream(ctx, Select(), Range(1, 5), func(e *Event) error {
			events = append(events, e)
			return nil
		})
		Expect(len(events)).To(Equal(2))
	...
	It("Subscribe adds a subscription", func() {
		...
		ctx := context.Background()
		_, _ = w.Stream().Subscribe(ctx, "test", Select(), func(e *Event) error { return nil })
	...
	It("Subscribe adds an subscription and cancelling the context eventually disables it", func() {
		...
		sub, _ := w.Stream().Subscribe(ctx, "test", Select(), func(e *Event) error { return nil })
		Expect(len(w.Stream().Subscriptions())).To(Equal(1))
		cancel()
	...
	It("Subscribe gets newly emitted Events", func() {
		...
		var counter int
		ctx := context.Background()
		_, _ = w.Stream().Subscribe(ctx, "test", Select(), func(e *Event) error {
			counter++
			return nil
		})
		w.Emit(w.Agg("test", "1"))
	...
	It("Subscribe gets selected newly emitted Events", func() {
		...
		var counter int
		ctx := context.Background()
		_, _ = w.Stream().Subscribe(ctx, "test", Select(SelectAggregate("test", "1")), func(e *Event) error {
			counter++
			return nil
		})
		w.Emit(w.Agg("test", "1"))
	...
	It("multiple Subscribers get all relevant events", func() {
		...
		var counter1 int
		ctx := context.Background()
		_, _ = w.Stream().Subscribe(ctx, "test1", Select(), func(e *Event) error {
			counter1++
			return nil
		})
		var counter2 int
		_, _ = w.Stream().Subscribe(ctx, "test2", Select(), func(e *Event) error {
			counter2++
			return nil
		})
